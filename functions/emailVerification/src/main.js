...
/**
 * ======================================================
 * Catalogy - Appwrite Function (Email Verifications)
 *
 * Objetivo:
 * - Manejar el envío, verificación y reenvío de correos de verificación
 * - Usar Appwrite REST directamente con fetch para mejor debug
 * - Logs detallados para facilitar troubleshooting
 *
 * Nota:
...
function assertFetchAvailable() {
  if (typeof globalThis.fetch !== "function") {
    throw new Error(
      "globalThis.fetch is not available. Use Node 18+ runtime in Appwrite Functions.",
    );
  }
}
...
function getAppwriteEndpoint(log) {
  ...
  // 3) Normaliza
  // Si viene localhost/127.0.0.1 -> cambiar a 'appwrite' (hostname interno Docker)
    ...
    if (action === "verify") {
      ...
      const tokenDocs = await listDocuments({
        ...
        queries: [
          ...
          { method: "equal", attribute: "used", values: [false] },
          { method: "equal", attribute: "invalidated", values: [false] },
          { method: "limit", values: [1] },
        ],
      });
    ...
    if (msg.toLowerCase().includes("fetch failed")) {
      hint = {
        ...
        check: [
          ...
          "3) Verifica que el contenedor runtime esté en la network 'appwrite'",
